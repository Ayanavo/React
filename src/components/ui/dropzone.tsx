...
const dropZoneErrorCodes = ["file-invalid-type", "file-too-large", "file-too-small", "too-many-files"] as const;

const getDropZoneErrorCodes = (fileRejections: FileRejection[]) => {
  const errors = fileRejections.map((rejection) => {
    return rejection.errors.filter((error) => dropZoneErrorCodes.includes(error.code as DropZoneErrorCode)).map((error) => error.code) as DropZoneErrorCode[];
  });
...
const useDropzone = <TUploadRes, TUploadError = string>(props: UseDropzoneProps<TUploadRes, TUploadError>): UseDropzoneReturn<TUploadRes, TUploadError> => {
  const {
...
interface DropzoneProps<TUploadRes, TUploadError> extends UseDropzoneReturn<TUploadRes, TUploadError> {
...
const DropZoneArea = forwardRef<HTMLDivElement, DropZoneAreaProps>(({ className, children, ...props }, forwardedRef) => {
...
const DropzoneDescription = forwardRef<HTMLParagraphElement, DropzoneDescriptionProps>((props, ref) => {
  ...
  return <p ref={ref} id={context.rootDescriptionId} {...rest} className={cn("pb-1 text-sm text-muted-foreground", className)} />;
});
...
const DropzoneFileList = forwardRef<HTMLOListElement, DropZoneFileListProps>((props, ref) => {
  ...
  return (
    <ol ref={ref} aria-label="dropzone-file-list" {...props} className={cn("flex flex-col gap-4", props.className)}>
      {props.children}
    </ol>
  );
});
...
interface DropzoneFileListItemProps<TUploadRes, TUploadError> extends React.LiHTMLAttributes<HTMLLIElement> {
  file: FileStatus<TUploadRes, TUploadError>;
}

const DropzoneFileListItem = forwardRef<HTMLLIElement, DropzoneFileListItemProps<unknown, unknown>>(({ className, ...props }, ref) => {
  const fileId = props.file.id;
  const { onRemoveFile: cOnRemoveFile, onRetry: cOnRetry, getFileMessageId: cGetFileMessageId, canRetry: cCanRetry, inputId: cInputId } = useDropzoneContext();
  ...
  return (
    <DropzoneFileListContext.Provider
      ...
      <li
        ...
        className={cn("flex flex-col justify-center gap-2 rounded-md bg-muted/40 px-4 py-2", className)}>
        {props.children}
      </li>
    </DropzoneFileListContext.Provider>
  );
...
const DropzoneFileMessage = forwardRef<HTMLParagraphElement, DropzoneFileMessageProps>((props, ref) => {
  const { children, ...rest } = props;
  ...
  return (
    <p ref={ref} id={context.messageId} {...rest} className={cn("h-5 text-[0.8rem] font-medium text-destructive", rest.className)}>
      {body}
    </p>
  );
});
...
const DropzoneMessage = forwardRef<HTMLParagraphElement, DropzoneMessageProps>((props, ref) => {
  ...
  return (
    <p ref={ref} id={context.rootMessageId} {...rest} className={cn("h-5 text-[0.8rem] font-medium text-destructive", rest.className)}>
      {body}
    </p>
  );
});
...
const DropzoneRemoveFile = forwardRef<HTMLButtonElement, DropzoneRemoveFileProps>(({ className, ...props }, ref) => {
  const context = useDropzoneFileListContext();
  if (!context) {
    throw new Error("DropzoneRemoveFile must be used within a DropzoneFileListItem");
  }
  return (
    <Button ref={ref} onClick={context.onRemoveFile} type="button" size="icon" {...props} className={cn("aria-disabled:pointer-events-none aria-disabled:opacity-50", className)}>
      {props.children}
...
const DropzoneRetryFile = forwardRef<HTMLButtonElement, DropzoneRetryFileProps>(({ className, ...props }, ref) => {
  const context = useDropzoneFileListContext();
...
const DropzoneTrigger = forwardRef<HTMLLabelElement, DropzoneTriggerProps>(({ className, children, ...props }, ref) => {
  ...
  const fileMessageIds = useMemo(() => fileStatuses.filter((file) => file.status === "error").map((file) => getFileMessageId(file.id)), [fileStatuses, getFileMessageId]);
  ...
  return (
    <label
      ...
      <input
        ...
        aria-describedby={context.isInvalid ? [context.rootMessageId, ...fileMessageIds].join(" ") : undefined}
...
const InfiniteProgress = forwardRef<HTMLDivElement, InfiniteProgressProps>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ...
      <div
        ...
        className={cn("h-full w-full rounded-full bg-primary", done ? "translate-x-0" : "animate-infinite-progress", error && "bg-destructive")}
      />
    </div>
  );
});

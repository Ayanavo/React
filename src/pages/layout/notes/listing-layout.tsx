...
function listinglayout({ setIsOpen, isOpen }: { setIsOpen: (arg: State) => void; isOpen: State }) {
  useEffect(() => {
  ...
  return (
    <>
      {noteListing.map((item: State, index: number) => (
        <Card key={index} className="m-3 shadow-none hover:shadow transition-shadow" onClick={() => setIsOpen(item)}>
